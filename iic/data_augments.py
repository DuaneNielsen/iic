...
def peturb(x, tps_theta, cntl_pts, theta_rotate, padding_mode='zeros'):
    x = tps_transform(x, tps_theta, cntl_pts, padding_mode=padding_mode)
    x = rotate_affine_grid_multi(x, theta_rotate, padding_mode=padding_mode)
    return x
...
class TpsAndRotateSecond(object):
    def __init__(self, data_aug_tps_cntl_pts, data_aug_tps_variance, data_aug_max_rotate, padding_mode='zeros'):
        self.tps_cntl_pts, self.tps_variance, self.max_rotate = data_aug_tps_cntl_pts, data_aug_tps_variance, data_aug_max_rotate
    ...
    def __call__(self, *data):
        ...
        theta_tps, cntl_pts, theta_rotate = rand_peturb_params(bsize, self.tps_cntl_pts, self.tps_variance, self.max_rotate)
        x_ = peturb(x, theta_tps, cntl_pts, theta_rotate, self.padding_mode)
        loss_mask = peturb(loss_mask, theta_tps, cntl_pts, theta_rotate)
        return x, x_, loss_mask